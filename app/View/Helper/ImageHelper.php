...
class ImageHelper extends AppHelper 
{
    ...
    function resize($file, $fullPath = null, $widthCrop = null, $heightCrop = null, $htmlAttributes = array(),$zoom=false) 
    {
        ...
        $relFile = '/img/uploads/cache/'.$widthCrop.'-'.$heightCrop.'.'.basename($file); // relative file
        $cacheFile = Configure::read('S.uploadDir.cache').'/'.$widthCrop.'-'.$heightCrop.'.'.basename($file);
        ...
        //check file existed
        if (!is_dir(Configure::read('S.uploadDir.cache'))) {
            mkdir(Configure::read('S.uploadDir.cache'), 0777, true);
        }
